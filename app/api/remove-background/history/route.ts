...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log(`Getting remove-background history for user: ${user.id}, limit: ${limit}, offset: ${offset}`);

    // 3. 构建查询 - 专门查询 remove_background 任务
    let query = supabaseAdmin
      ...
      `)
      .eq('user_id', user.id)
      .eq('engine', 'remove_background') // 只查询去除背景任务
    ...
    if (queryError) {
      console.error('Error querying remove-background history:', queryError);
      return apiResponse.serverError('获取去除背景历史记录失败');
    }
    ...
    if (countError) {
      console.error('Error counting remove-background history:', countError);
    }
    ...
    console.error('Error in remove-background/history API:', error);
    return apiResponse.serverError('获取去除背景历史记录失败');
  }
}