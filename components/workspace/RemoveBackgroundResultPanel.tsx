...
interface RemoveBackgroundResultPanelProps {
...
export default function RemoveBackgroundResultPanel({
  taskStatus,
  isProcessing,
  onDownload,
  onCopyUrl,
}: RemoveBackgroundResultPanelProps) {
        ...
        {!taskStatus && !isProcessing ? (
          <div className="flex-1 rounded-lg flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-400">{t('status.waiting')}</p>
            </div>
          </div>
        ) : isProcessing ? (
          <div className="flex-1 rounded-lg flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-400">{t('messages.processingDescription')}</p>
            </div>
          </div>
                ...
                <div className="flex-1 min-h-0 flex flex-col justify-between">
                  <div className="flex justify-center flex-1 items-center">
                    <div className="w-full max-w-2xl lg:max-w-3xl">
                      <div className="relative w-full mx-auto rounded-lg overflow-hidden shadow-2xl max-h-[60vh]" 
                  ...
                  <div className="bg-gray-800/50 rounded-xl p-3 space-y-2 mx-auto max-w-2xl lg:max-w-3xl w-full mt-4 flex-shrink-0">
                    ...
                    <div className="border-t border-gray-700 pt-3">
                      <h5 className="text-sm font-medium text-gray-300 mb-2">{t('taskDetails.title')}</h5>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                        <div className="flex justify-between md:flex-col md:items-start">
                          <span className="text-gray-400">{t('taskDetails.status')}</span>
                        ...
                        <div className="flex justify-between md:flex-col md:items-start">
                          <span className="text-gray-400">{t('taskDetails.creditsConsumed')}</span>
                          <span className="text-cyan-400 font-semibold">{taskStatus.creditsConsumed}</span>
                        </div>
                        <div className="flex justify-between md:flex-col md:items-start">
                          <span className="text-gray-400">{t('taskDetails.completedTime')}</span>
          ...
          <div className="flex-1 rounded-lg flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-red-300 max-w-md">{taskStatus.error || t('messages.backgroundRemovalFailed')}</p>
            </div>
          </div>
        ) : null}
      </div>
